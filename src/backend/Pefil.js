...
app.post('/api/logout', async (req, res) => {
  ...
  try {
    ...
    connection.query(
      "UPDATE usuario SET sesionActiva = 0 WHERE email = ?",
      [email],
      (err, results) => {
        connection.end()
...
app.delete('/api/eliminarCuenta', async (req, res) => {
  ...
  try {
    ...
    connection.query(
      ...
      (err, results) => {
        connection.end()
        if (err) return res.status(500).json({ message: "Error eliminando la cuenta" })
...
export async function POST(request) {
  try {
    ...
    */
    ...
    const usuarioActualizado = await actualizarUsuarioEnBaseDeDatos(userId, datosActualizados)
